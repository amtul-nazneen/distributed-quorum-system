...
/**
 * Central class that handlers Client 4 actions
 */
public class Client4 {

	/*
	 * MutualExclusionImpl myMutexImpl; int processnum = 4; int counter = 0;
	 * 
	 * Socket server1 = null, server2 = null, server3 = null; Socket s2, s1, s3, s5;
	 * ServerSocket ss5;
	 * 
	 * PrintWriter w1, w2, w3, w5; BufferedReader r1, r2, r3, r5; PrintWriter
	 ...
	 *//**
		 ...
		 * 1,2,3 and initialises Channel Readers and Writers. Opens connections for the
		 * remaining 1 Client and initialises Channel Readers and Writers once connected
		 * Creates a Mutex object that handles the RA Algorithm and optimization
		 * Generates 20 Random Requests
		 * 
		 * @throws Exception
		 */
	/*
	 * public void startClient4() throws Exception { Timestamp start =
	 * Utils.getTimestamp(); try { connectToServer(); connectToOtherClients();
	 * createServerIOStream(); createChannelIOStream(); createMutexImplementor();
	 * startChannelThreads(); init(); while (counter < Constants.CLIENT4_CSLIMIT) {
	 * try { setRandomRequestParams(); requestForCSaccess(); counter++;
	 * Thread.sleep((long) (Math.random() * 1000)); } catch (Exception e) {
	 * Utils.log(e.getMessage()); } }
	 * 
	 * } catch (Exception e) { Utils.log(e.getMessage());
	 * 
	 * } finally { if (Constants.ENABLE_SOCKET_CLOSE) { while
	 * (Utils.checkTimeout(start, Utils.getTimestamp()) <= Constants.CLIENT_TIMEOUT
	 * + 2) { Thread.sleep(30000); } Utils.log("Closing all sockets");
	 * closeAllSockets(); } } }
	 * 
	 *//**
	 ...
	 * Thread.sleep(Constants.CLIENT4_CSEXEC); } catch (InterruptedException e) {
	 * e.printStackTrace(); }
	 ...
	 * writeToServer3.println(Constants.ENQUIRE + "," + processnum); boolean
	 * gotReply = false; String reply = ""; while (!gotReply) { reply =
	 * readFromServer3.readLine(); if (reply != null) { gotReply = true; } }
	 ...
	 * Socket(Constants.QUORUM2_HOST, Constants.SERVER_PORT); server3 = new
	 * Socket(Constants.QUORUM3_HOST, Constants.SERVER_PORT);
	 * 
	 * }
	 * 
	 *//**
	 ...
	 * Socket(Constants.CLIENT1, Constants.CLIENT4_PORT); // s2 = new
	 * Socket(Constants.CLIENT2, Constants.CLIENT4_PORT); // s3 = new
	 * Socket(Constants.CLIENT3, Constants.CLIENT4_PORT); // ss5 = new
	 * ServerSocket(Constants.CLIENT5_PORT); s5 = ss5.accept(); }
	 * 
	 *//**
	 ...
	 * InputStreamReader(s1.getInputStream())); w2 = new
	 ...
	 * InputStreamReader(s3.getInputStream())); w5 = new
	 ...
	 * QuorumClientHandler(s1, myMutexImpl); QuorumClientHandler css2 = new
	 * QuorumClientHandler(s2, myMutexImpl); QuorumClientHandler css3 = new
	 * QuorumClientHandler(s3, myMutexImpl); QuorumClientHandler css5 = new
	 * QuorumClientHandler(s5, myMutexImpl); Thread t1 = new Thread(css1); Thread t2
	 * = new Thread(css2); Thread t3 = new Thread(css3); Thread t5 = new
	 * Thread(css5); t1.start(); t2.start(); t3.start(); t5.start(); }
	 * 
	 *//**
	 ...
	 *//**
		 ...
		 *//*
			 ...
			 * server3.close(); s1.close(); s2.close(); s3.close(); s5.close(); ss5.close();
			 * } catch (Exception e) { } }
			 */
}
